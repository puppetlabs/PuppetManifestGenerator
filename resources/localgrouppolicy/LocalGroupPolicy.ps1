Function Get-LocalGroupPolicy {
  ...
  Process {
    ...
    $PolFileEditorCS = @"
//FROM https://gallery.technet.microsoft.com/scriptcenter/Read-or-modify-Registrypol-778fed6e
...
"@
    # TODO Import based on PS Version
    ...
    "Machine","User" | % {
      $PolicyType = $_
      $PolicyPath = "$($env:systemroot)\system32\GroupPolicy\$($PolicyType)\registry.pol"

      if (Test-Path -Path $PolicyPath) {
        $objPolicy = New-Object TJX.PolFileEditor.PolFile
        $objPolicy.LoadFile($PolicyPath)
        Write-Verbose "Opened the registry pol file at $PolicyPath"
        
        if ($objPolicy.Entries -ne $null) {
          $objPolicy.Entries | % {
            $objEntry = $_
            ...
            switch ($objEntry.Type) {           
              "REG_DWORD" { $props.value = $objEntry.DWORDValue}
              # TODO QWORD,MULTISZ,BINARY
              default { $props.value = $objEntry.StringValue}
            }
            
            Write-Output (New-Object -TypeName PSCustomObject -Property $props)
          }
        }
      } else {
        Write-Verbose "Could not find registry pol file at $PolicyPath"
      }
    }
  }
}